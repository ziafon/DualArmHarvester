...
from config import _JointName, motorParam, jointReachThreshold


v = motorParam['vertical']
h = motorParam['horizontal']
e = motorParam['extend']
maxVelocity = np.array([20, 20, 20, 20, 20, 20, 15, 15, 15, 15]) * 100000



# 这里面又出现了a，b命名的字典，oz oy同样不知所云
pulse2meter = np.array([v, v, h, h, h, h, e, e, e, e])  
...
bitArray = np.array([128, 64, 32, 16, 8, 4, 2])


def adaptiveControl(delta, threshold):
    """
...
def Int2Byte(data):
    byteString = bin(data)[2:].zfill(16)    
...
def invCalculate(coordinate, tf_listener, id, dir):
    ...
    fake_z = fake_z - fake_y * oz + 0.02
    fake_y = fake_y / oy + 0.05
    if id == 'DL' or id == 'DR':
...
def single_adaptive(delta, threshold):
    # print(delta)
    deltaABS = np.abs(delta)
    direction = np.sign(delta)
    # velocity = np.floor(np.exp(deltaABS-threshold)*0.5) * 1500000 + np.sqrt(deltaABS) * 1000000
