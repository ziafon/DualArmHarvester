...
class Schedule():
    ...
    def __init__(self, config):
        ...
        self.jointNum = config['jointNum']

        self.jsPosition = [0.0 for _ in range(config['urdfJointNum'])]
    ...
    def motorFeedback(self):
        rospy.Subscriber('motor_feedback', hardware, callback=self.robot_callback ,queue_size=3)
        rospy.spin()
    ...
    def robot_callback(self, msgs):
        ...
        self.joint_positon, self.currentPosition = unitConvertion(self.joint_positon, self.motor_position)
        # 计算关键距离，共三个元素。0：上下导轨之间的距离， 1：左下和右下两臂横向距离，2：左上和右上两臂横向距离
